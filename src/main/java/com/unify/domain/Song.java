...
public class Song {
	...
	public Song(String title, String artistName, int duration,
			String songUrl, String imageUrl)
			throws InvalidParamException {
		
		// if any invalid parameter throws InvalidParamException
		if (isAnyStringVoid(title, artistName, songUrl, imageUrl)
				|| duration <= 0) {
	...
	private static boolean isAnyStringVoid(String... stringsToCheck) {
	...
	public boolean hasTimeLeft() {
		int timePlaying = getSecDifference(
				Calendar.getInstance().getTime(), dateOfCurrentSong);
