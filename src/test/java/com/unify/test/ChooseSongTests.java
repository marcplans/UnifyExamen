...
public class ChooseSongTests extends AbstractTests {
	// FEATURE: As a user, when there is no current song in the university,
	// I want to choose a new song so all other users can listen to it.
	...
	public void isn_t_playing_any_song(String uniId)
			throws Throwable {
		clearBBDD();
	...
	public void have_these_songs_on_its_store(List<SongDTO> songs)
			throws Throwable {
		for (SongDTO s : songs) {
	...
	public void user_searches_for_all_songs_of_university(
			String uniId) throws Throwable {
		allSongs = getAllSongsUniversity(uniId);
	}

	@Then("^will return next songs:$")
	public void will_return_next_songs(List<SongDTO> songs)
			throws Throwable {
		for (SongDTO s : songs) {
	...
	public void that_is_not_playing_any_song(String uniId)
			throws Throwable {
		clearBBDD();
	...
	public void has_list_of_songs(List<SongDTO> songs)
			throws Throwable {
	...
	public void must_be_playing(String uniId, List<SongDTO> songs)
			throws Throwable {
		SongDTO currentSong = searchForCurrentSong(uniId);
		assertThat("Incorrect song playing",
				songs.get(0).equals(currentSong));
	}

	@Given("^university \"([^\"]*)\" is not playing any song$")
	public void university_is_not_playing_any_song(String uniId)
			throws Throwable {
		clearBBDD();
		university = tryToCreateUniversity(uniId);
	}
	...
	public void tries_to_add_these_songs(List<SongDTO> songs)
			throws Throwable {
		for (SongDTO song : songs) {
			responses.add(service.addSongsInUniversityResponseCode(
					song, university.getId()));
		}
	}
